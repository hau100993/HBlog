...
* Factory Design Example (by ChatGPT):
...
class ButtonFactory {
public:
    static Button* createButton(const std::string& platform) {
        if (platform == "Windows") {
            return new WindowsButton;
        } else if (platform == "MacOS") {
            return new MacOSButton;
        } else {
            // Throw an exception or return a default button
        }
    }
};
...
* Astract Factory Design Example (by ChatGPT):
